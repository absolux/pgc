...
class ExtendedStatement extends AbstractStatement {
  ...
  _listenExecuteEvents () {
    ...
    const onPortalSuspended = () => {
      this._connection.write(Buffer.concat([
        createExecuteMessage(this._potral),
        FLUSH_MESSAGE
      ]))
    }
    ...
    con.on('PG:PortalSuspended', onPortalSuspended)
    ...
    con.once('PG:ReadyForQuery', () => {
      ...
      con.removeListener('PG:PortalSuspended', onPortalSuspended)
